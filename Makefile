...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend -t Oality/oality-website-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend -t Oality/oality-website-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name oality-website-backend-acceptance -d Oality/oality-website-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name oality-website-frontend-acceptance --link oality-website-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d Oality/oality-website-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop oality-website-frontend-acceptance
	@docker stop oality-website-backend-acceptance
